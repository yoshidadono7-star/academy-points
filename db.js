...
const TeacherCommentsDB = {
  async add(data) {
    const ref = await db.collection('teacherComments').add({
      ...
      method: data.method || 'text', // text, voice, auto, ai
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    });
    return ref.id;
  },
  async getByStudentDate(studentId, date) {
    const snap = await db.collection('teacherComments')
      .where('studentId', '==', studentId)
      .where('date', '==', date).get();
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  },
  async getByStudentRange(studentId, startDate, endDate) {
    const snap = await db.collection('teacherComments')
      .where('studentId', '==', studentId)
      .where('date', '>=', startDate)
...
// テンプレート自動コメント生成
// ============================================
function generateAutoComment(student, todayLogs, condition) {
  const parts = [];
  const name = student.nickname || student.callsign || 'お子さん';

  if (todayLogs && todayLogs.length > 0) {
    const totalMin = todayLogs.reduce((s, l) => s + (l.duration || 0), 0);
    const subjects = [...new Set(todayLogs.map(l => l.subject).filter(Boolean))];
    if (subjects.length > 0) {
      parts.push(`${name}さんは本日${subjects.join('・')}に${totalMin}分取り組みました。`);
    } else {
      parts.push(`${name}さんは本日${totalMin}分学習しました。`);
    }
    if (todayLogs.some(l => l.method === 'pomodoro')) {
      const pomoCount = todayLogs.reduce((s, l) => s + (l.pomodoroCount || 0), 0);
      if (pomoCount > 0) parts.push(`ポモドーロ${pomoCount}回達成。`);
    }
  } else {
    parts.push(`${name}さんの本日の学習記録です。`);
  }

  if (condition) {
    const mood = condition.mood || 3;
    const motivation = condition.motivation || 3;
    const energy = condition.energy || 3;
    if (motivation >= 4 && energy >= 4) parts.push('やる気十分で集中して学習できていました。');
    else if (motivation >= 4) parts.push('意欲的に取り組んでいました。');
    else if (motivation <= 2 && energy <= 2) parts.push('少し疲れ気味でしたが、最後まで頑張りました。');
    else if (motivation <= 2) parts.push('少しモチベーションが下がり気味でしたが、取り組んでいました。');
    else parts.push('落ち着いて学習に取り組んでいました。');
    if (condition.note) parts.push(`（本人より：${condition.note}）`);
  }

  const streak = student.streak || 0;
  if (streak >= 14) parts.push(`${streak}日連続通塾中です。素晴らしい継続力です！`);
  else if (streak >= 7) parts.push(`${streak}日連続通塾中です。良いリズムが続いています。`);
  else if (streak >= 3) parts.push(`${streak}日連続で通塾しています。`);
...
// ============================================
const ReportEngine = {
  async generateDaily(studentId) {
    const student = await StudentsDB.getById(studentId);
    if (!student) return null;
    const today = getToday();
    const name = student.nickname || student.callsign || student.name;

    const [logs, conditions, comments, badges] = await Promise.all([
      StudyLogsDB.getByStudent(studentId, 10),
      ConditionsDB.getByStudent(studentId, 3),
      TeacherCommentsDB.getByStudentDate(studentId, today),
      BadgesDB.getByStudent(studentId)
    ]);

    const todayLogs = logs.filter(l => l.date === today);
    const todayCondition = conditions.find(c => c.date === today);
    const totalMin = todayLogs.reduce((s, l) => s + (l.duration || 0), 0);
    const subjects = [...new Set(todayLogs.map(l => l.subject).filter(Boolean))];
    const points = todayLogs.reduce((s, l) => s + (l.points || 0), 0);
    const teacherComment = comments.length > 0 ? comments[0].comment : '';
    const autoComment = generateAutoComment(student, todayLogs, todayCondition);

    const text = [
      `【HERO'S】${name}さんの本日の学習レポート`,
      '',
      `📅 ${today}`,
      todayLogs.length > 0 ? `✅ 出席 | ${totalMin}分 | ${subjects.join('・')}` : '❌ 欠席',
      points > 0 ? `⭐ +${points}ポイント` : '',
      todayCondition ? `💚 気分${todayCondition.mood}/5 やる気${todayCondition.motivation}/5` : '',
      '',
      '📝 先生より:',
      teacherComment || autoComment || '（コメントなし）'
    ].filter(Boolean).join('\n');
    ...
    const html = `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:20px;border-radius:12px 12px 0 0;">
        <h2 style="margin:0;">📋 ${name}さんの学習レポート</h2>
        <p style="margin:4px 0 0;opacity:0.9;">${today}</p>
      </div>
      <div style="padding:20px;background:#fff;border:1px solid #e2e8f0;">
        ${todayLogs.length > 0 ? `
          <div style="display:flex;gap:16px;margin-bottom:16px;">
            <div style="text-align:center;flex:1;background:#f0f4ff;padding:12px;border-radius:8px;">
              <div style="font-size:24px;font-weight:900;color:#4c51bf;">${totalMin}</div><div style="font-size:11px;color:#666;">分</div>
            </div>
            <div style="text-align:center;flex:1;background:#f0fdf4;padding:12px;border-radius:8px;">
              <div style="font-size:24px;font-weight:900;color:#059669;">+${points}</div><div style="font-size:11px;color:#666;">ポイント</div>
            </div>
            <div style="text-align:center;flex:1;background:#fef3c7;padding:12px;border-radius:8px;">
              <div style="font-size:24px;font-weight:900;color:#92400e;">🔥${student.streak || 0}</div><div style="font-size:11px;color:#666;">連続日</div>
            </div>
          </div>
          <p style="font-size:14px;color:#333;">📚 科目: ${subjects.join('、')}</p>
        ` : '<p style="color:#ef4444;font-weight:700;">本日は欠席でした</p>'}
        ${todayCondition ? `<p style="font-size:13px;color:#666;">💚 気分: ${'😢😟😐😊🤩'.split('')[todayCondition.mood-1] || '😐'} | やる気: ${todayCondition.motivation}/5 | 元気: ${todayCondition.energy}/5</p>` : ''}
        <div style="margin-top:16px;padding:14px;background:#f8fafc;border-left:3px solid #667eea;border-radius:0 8px 8px 0;">
          <p style="font-size:12px;color:#667eea;font-weight:700;margin:0 0 6px;">📝 先生より</p>
          <p style="font-size:14px;color:#333;line-height:1.7;margin:0;">${teacherComment || autoComment || '（コメントなし）'}</p>
        </div>
      </div>
      <div style="padding:12px;background:#f8fafc;border-radius:0 0 12px 12px;text-align:center;font-size:11px;color:#94a3b8;border:1px solid #e2e8f0;border-top:none;">
        HERO'S ACADEMY より自動送信
      </div>
    </div>`;

    return { subject: `【HERO'S】${name}さんの学習レポート（${today}）`, html, text, type: 'daily', studentId, data: { totalMin, subjects, points, streak: student.streak, teacherComment } };
  },

  async generateWeekly(studentId) {
    const student = await StudentsDB.getById(studentId);
    if (!student) return null;
    const name = student.nickname || student.callsign || student.name;
    const today = new Date();
    ...
    const [logs, conditions, comments] = await Promise.all([
      StudyLogsDB.getByDateRange(startStr, endStr),
      ConditionsDB.getByDateRange(startStr, endStr),
      TeacherCommentsDB.getByStudentRange(studentId, startStr, endStr)
    ]);

    const studentLogs = logs.filter(l => l.studentId === studentId);
    const studentCond = conditions.filter(c => c.studentId === studentId);
    const totalMin = studentLogs.reduce((s, l) => s + (l.duration || 0), 0);
    const studyDays = new Set(studentLogs.map(l => l.date)).size;
    const subjects = {};
    studentLogs.forEach(l => { subjects[l.subject] = (subjects[l.subject] || 0) + (l.duration || 0); });
    const latestComment = comments.length > 0 ? comments[0].comment : '';

    const text = [
      `【HERO'S】${name}さんの週次レポート`,
      `📅 ${startStr} 〜 ${endStr}`,
      `📊 学習${studyDays}日 / ${totalMin}分`,
      `📚 ${Object.entries(subjects).map(([k, v]) => `${k}:${v}分`).join(' ')}`,
      `🔥 連続${student.streak || 0}日`,
      latestComment ? `📝 先生より: ${latestComment}` : ''
    ].filter(Boolean).join('\n');

    const html = `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:20px;border-radius:12px 12px 0 0;">
        <h2 style="margin:0;">📊 ${name}さんの週次レポート</h2>
        <p style="margin:4px 0 0;opacity:0.9;">${startStr} 〜 ${endStr}</p>
      </div>
      <div style="padding:20px;background:#fff;border:1px solid #e2e8f0;">
        <div style="display:flex;gap:12px;margin-bottom:16px;">
          <div style="text-align:center;flex:1;background:#f0f4ff;padding:12px;border-radius:8px;">
            <div style="font-size:24px;font-weight:900;color:#4c51bf;">${studyDays}</div><div style="font-size:11px;">学習日</div>
          </div>
          <div style="text-align:center;flex:1;background:#f0f4ff;padding:12px;border-radius:8px;">
            <div style="font-size:24px;font-weight:900;color:#4c51bf;">${totalMin}</div><div style="font-size:11px;">分</div>
          </div>
          <div style="text-align:center;flex:1;background:#fef3c7;padding:12px;border-radius:8px;">
            <div style="font-size:24px;font-weight:900;color:#92400e;">🔥${student.streak || 0}</div><div style="font-size:11px;">連続日</div>
          </div>
        </div>
        <h3 style="font-size:14px;margin-bottom:8px;">📚 科目別学習時間</h3>
        ${Object.entries(subjects).map(([subj, min]) => {
          const pct = Math.round(min / totalMin * 100);
          return `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;font-size:13px;"><span>${subj}</span><span>${min}分 (${pct}%)</span></div><div style="background:#e2e8f0;border-radius:4px;height:8px;"><div style="background:linear-gradient(90deg,#667eea,#764ba2);height:100%;width:${pct}%;border-radius:4px;"></div></div></div>`;
        }).join('')}
        ${studentCond.length > 0 ? `<h3 style="font-size:14px;margin:16px 0 8px;">💚 コンディション推移</h3><div style="display:flex;gap:4px;">${studentCond.sort((a,b)=>a.date.localeCompare(b.date)).map(c => `<div style="text-align:center;padding:4px 6px;background:#f8fafc;border-radius:6px;flex:1;"><div style="font-size:9px;color:#94a3b8;">${c.date.slice(5)}</div><div style="font-size:16px;">${'😢😟😐😊🤩'.split('')[c.mood-1]||'😐'}</div></div>`).join('')}</div>` : ''}
        ${latestComment ? `<div style="margin-top:16px;padding:14px;background:#f8fafc;border-left:3px solid #667eea;border-radius:0 8px 8px 0;"><p style="font-size:12px;color:#667eea;font-weight:700;margin:0 0 6px;">📝 先生より</p><p style="font-size:14px;color:#333;line-height:1.7;margin:0;">${latestComment}</p></div>` : ''}
      </div>
      <div style="padding:12px;background:#f8fafc;border-radius:0 0 12px 12px;text-align:center;font-size:11px;color:#94a3b8;border:1px solid #e2e8f0;border-top:none;">HERO'S ACADEMY より自動送信</div>
    </div>`;

    return { subject: `【HERO'S】${name}さんの週次レポート（${startStr}〜）`, html, text, type: 'weekly', studentId };
  },

  async generateMonthly(studentId) {
    const student = await StudentsDB.getById(studentId);
    if (!student) return null;
    const name = student.nickname || student.callsign || student.name;
    const now = new Date();
    ...
    const [logs, conditions, examResults, comments] = await Promise.all([
      StudyLogsDB.getByDateRange(startStr, endStr),
      ConditionsDB.getByDateRange(startStr, endStr),
      ExamResultsDB.getByStudent(studentId),
      TeacherCommentsDB.getByStudentRange(studentId, startStr, endStr)
    ]);
    ...
    const text = [
      `【HERO'S】${name}さんの月次レポート`,
      `📅 ${year}年${month + 1}月`,
      `📊 学習${studyDays}日 / ${Math.round(totalMin / 60)}時間`,
      `Lv.${student.level || 1} | ${(student.totalPoints || 0).toLocaleString()}pt`,
      latestComment ? `📝 先生より: ${latestComment}` : ''
    ].filter(Boolean).join('\n');

    const html = `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:20px;border-radius:12px 12px 0 0;">
        <h2 style="margin:0;">📋 ${name}さんの月次レポート</h2>
        <p style="margin:4px 0 0;opacity:0.9;">${year}年${month + 1}月</p>
      </div>
      <div style="padding:20px;background:#fff;border:1px solid #e2e8f0;">
        <div style="display:flex;gap:12px;margin-bottom:16px;">
          <div style="text-align:center;flex:1;background:#f0f4ff;padding:14px;border-radius:8px;">
            <div style="font-size:28px;font-weight:900;color:#4c51bf;">${studyDays}</div><div style="font-size:11px;">学習日</div>
          </div>
          <div style="text-align:center;flex:1;background:#f0f4ff;padding:14px;border-radius:8px;">
            <div style="font-size:28px;font-weight:900;color:#4c51bf;">${Math.round(totalMin / 60)}</div><div style="font-size:11px;">時間</div>
          </div>
          <div style="text-align:center;flex:1;background:#fef3c7;padding:14px;border-radius:8px;">
            <div style="font-size:28px;font-weight:900;color:#92400e;">Lv.${student.level || 1}</div><div style="font-size:11px;">${(student.totalPoints || 0).toLocaleString()}pt</div>
          </div>
        </div>
        ${latestComment ? `<div style="margin-top:16px;padding:14px;background:#f8fafc;border-left:3px solid #667eea;border-radius:0 8px 8px 0;"><p style="font-size:12px;color:#667eea;font-weight:700;margin:0 0 6px;">📝 先生より</p><p style="font-size:14px;color:#333;line-height:1.7;margin:0;">${latestComment}</p></div>` : ''}
      </div>
    ...
    return { subject: `【HERO'S】${name}さんの月次レポート（${year}年${month + 1}月）`, html, text, type: 'monthly', studentId };
  },

  async generateInstant(type, data) {
    const messages = {
      schedule_approved: { subject: '【HERO'S】スケジュールが承認されました', text: `${data.date} ${data.startTime}〜${data.endTime} の予定が承認されました。` },
      schedule_rejected: { subject: '【HERO'S】スケジュールが差し戻されました', text: `${data.date} ${data.startTime}〜${data.endTime} が差し戻されました。${data.reason ? '理由: ' + data.reason : ''}` },
      badge_earned: { subject: '【HERO'S】新しいバッジを獲得！', text: `${data.badgeName}バッジを獲得しました！ ${data.badgeDesc}` },
      risk_detected: { subject: '【HERO'S】学習状況のお知らせ', text: `お子さんの学習状況に変化がありました。${data.factors ? data.factors.map(f => f.label).join('、') : ''}` },
      exam_result: { subject: '【HERO'S】テスト結果が登録されました', text: `${data.examName}: ${data.totalScore}点` }
    };
    const msg = messages[type] || { subject: '【HERO'S】お知らせ', text: data.message || '' };
    return { ...msg, html: `<p>${msg.text}</p>`, type: 'instant', studentId: data.studentId };
  }
};
...
const NotificationChannel = {
  async send(studentId, report) {
    const student = await StudentsDB.getById(studentId);
    if (!student) return { error: 'student not found' };
    const prefs = student.notifyPrefs || { email: true, line: true, app: true };
    const results = {};

    // 1. アプリ内通知
    if (prefs.app !== false) {
      try {
        await NotificationsDB.add({ type: report.type, title: report.subject, message: report.text, studentId });
        results.app = 'sent';
      } catch (e) { results.app = 'error: ' + e.message; }
    }

    // 2. メール（EmailJS）
    if (prefs.email !== false && student.parentContact && student.parentContact.includes('@')) {
      try {
        const emailConfig = JSON.parse(localStorage.getItem('heros_emailjs') || '{}');
        if (emailConfig.serviceId && emailConfig.templateId && emailConfig.publicKey) {
          await emailjs.send(emailConfig.serviceId, emailConfig.templateId, {
            ...
            from_name: "HERO'S ACADEMY"
          }, emailConfig.publicKey);
          results.email = 'sent';
        } else { results.email = 'skip: EmailJS未設定'; }
      } catch (e) { results.email = 'error: ' + e.message; }
    }

    // 3. LINE Notify
    if (prefs.line !== false && student.lineNotifyToken) {
      try {
        const proxyUrl = localStorage.getItem('heros_line_proxy') || '';
        if (proxyUrl) {
          await fetch(proxyUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: student.lineNotifyToken, message: report.text })
          });
          results.line = 'sent';
        } else { results.line = 'skip: LINEプロキシ未設定'; }
      } catch (e) { results.line = 'error: ' + e.message; }
    }

    // ログ記録
    try {
      await db.collection('reportLogs').add({
        studentId, type: report.type, subject: report.subject,
        channels: results,
  ...
  async sendBulk(studentIds, reportGenerator) {
    const results = [];
    for (const sid of studentIds) {
      try {
        const report = await reportGenerator(sid);
        if (report) {
          const r = await this.send(sid, report);
          results.push({ studentId: sid, success: true, channels: r });
        }
      } catch (e) { results.push({ studentId: sid, success: false, error: e.message }); }
    }
    return results;
  }
};
