 ...
 * awardStudyReward
 *
 * academy-points で生徒がポモドーロを完走した瞬間に呼ぶ。
 * 生徒本人のIDトークンで呼ばれる想定 (request.auth.uid === studentId)。
 *
 * 入力: { studentId, pomodoroCount, totalMinutes, subject? }
 * 処理:
 *   - rpg_profiles/{uid}    : xp 加算 + レベルアップ判定 (+ 初回は自動作成)
 *   - rpg_wallet/{uid}      : gold 加算 (+ 初回は自動作成)
 *   - rpg_gacha_tickets/{uid}: standard 加算 (+ 初回は自動作成)
 *   - rpg_daily_stats/{uid}/days/{YYYY-MM-DD(JST)}: minutes + pomodoros 加算
 *   - rpg_transactions      : 監査ログ
...
module.exports = onCall(async (request) => {
  ...
  // --- バリデーション ---
  const { studentId, pomodoroCount, totalMinutes, subject } = data;

  if (typeof studentId !== 'string' || studentId !== uid) {
    throw new HttpsError('permission-denied', 'studentId must match authenticated uid.');
  }
  ...
  if (pomodoroCount < 1 || pomodoroCount > MAX_POMODOROS_PER_CALL) {
    throw new HttpsError(
      'invalid-argument',
      `pomodoroCount must be between 1 and ${MAX_POMODOROS_PER_CALL}.`
    );
  }
  if (typeof totalMinutes !== 'number' || totalMinutes < 0 || totalMinutes > MAX_MINUTES_PER_CALL) {
    throw new HttpsError(
      'invalid-argument',
      `totalMinutes must be between 0 and ${MAX_MINUTES_PER_CALL}.`
    );
  }
  ...
  const dailyRef   = db.collection('rpg_daily_stats').doc(uid)
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (ticketsDoc.exists) {
      ...
      tx.set(ticketsRef, {
        standard: ticketsGained,
        premium: 0,
        daily_free_available: true,
        daily_free_last_used: null,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });
    }

    // Daily stats (per JST date)
    if (dailyDoc.exists) {
      tx.update(dailyRef, {
