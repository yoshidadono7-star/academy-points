...
// admin (Firebase Auth) でログインしている時だけ動く。
// → teacher.html / student.html / parent.html (PIN ログイン) では権限不足で
//   失敗するため、index.html (Firebase Auth) からのみ実行する。
// 起動時に1回だけ動く（settings/migrations.classroomIdAdded=true で再実行を防ぐ）
document.addEventListener('DOMContentLoaded', () => {
  // db.js が読み込まれているかチェック
  if (typeof migrateClassroomId !== 'function' || typeof ClassroomsDB === 'undefined') return;
  if (typeof firebase === 'undefined' || !firebase.auth) return;

  // Firebase Auth の状態を待つ
  firebase.auth().onAuthStateChanged(user => {
    if (!user) return; // 未ログインなら何もしない（PIN ログイン画面の場合）
    setTimeout(async () => {
      try {
